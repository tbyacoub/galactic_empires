...
class AttackPlanet implements ShouldQueue
{
    ...
    public function __construct($attackers, $attackID, $deffendID) //attckers will come in with travel->fleet, make a new travel back with the returning ships
    {
        ...
        $this->defender[0] = $planet->fleet('babylon5')->first()->count;
        $this->defender[1] = $planet->fleet('battlestar_galactica')->first()->count;
        $this->defender[2] = $planet->fleet('stargate')->first()->count;
        for($i = 0; $i < 3; $i++)
        {
            $this->healthAtt[$i] = $planet->fleet($fleet[$i])->first()->health;
